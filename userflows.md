...
A look asks the situation at an instant. It adds nothing to the log. Both seats look at the same log, so they get the same situation. They may look at different instants, and then the clock they each see can differ.

An attempt names an act, the seat it is made for, and the instant. It is admitted or it is not.
...
- Not admitted: the log is unchanged. The seat that attempted can tell. The other seat has nothing new to see.
...
- Whether this game counts as an input to a rating pool.
...
**Decline.** The other seat attempts a decline. Both look and see no offer standing. The game is not over.
...
**Offer, then play.** The seat that offered then attempts a play, and it is admitted. Both look and see the offer still standing, and the turn handed over. When that play is the one that makes the situation occur for the third time, both look and see a draw. The other seat did not have to accept.
...
**Claim that does not hold.** They named a play, and the claim does not hold. The play is admitted anyway when the situation admits it. Both look and see the turn handed over, and no draw. Nothing is added to the clock for the failed claim. With no play named, a claim that does not hold changes nothing.
...
**Resign.** They attempt a resignation, and both seats have already played. Both look and see a loss for the seat that resigned.
...
**Flag.** It is this seat's turn. Their time reaches zero. Nobody attempts a flag. Someone looks at a later instant, or this seat attempts a play after the time is gone. Both look and see a loss for this seat. The late play is not in the log.
...
**Counts.** They asked for rated, the two people are different, both seats have played, and the ending is not an abort and not still open. A look says this game is an input to the pool of its speed. The new rating is not something this game stores.
