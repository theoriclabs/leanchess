...
- Threefold. The player to move claims a draw because this position is about to appear for the third time on a move they indicate, or has just appeared and it is their move. (FIDE 9.2) The claim is an act. That the positions match is `derived` from the log: same side to move, the same kinds and colors on the same squares, and the same moves available. En passant available in one and not the other makes them different. A castling right that has since been lost makes them different. (FIDE 9.2.3) The repeated thing is the position, not a run of moves, and the occurrences need not be consecutive.
